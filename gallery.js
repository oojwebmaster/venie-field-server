...
/** 작품명 길이. ★ 클라이언트(`config.world.easel.sign.titleMax`)와 **같아야**
 *  합니다 — 다르면 화면에서는 다 쳤는데 서버가 잘라 내고, 그 사실은
 *  아무 데도 안 남습니다. */
const TITLE_MAX = Number(process.env.ART_TITLE_MAX || 10);

/** 작품명 다듬기 — 보이는 글자이므로 방 이름과 같은 잣대로 */
export function sanitizeTitle(raw, max = TITLE_MAX) {
...
export function isImage(v) {
  ...
  /* ★ jpeg는 받지 않습니다 — **투명이 없어서** 안 칠한 곳이 색으로
   *   메워집니다. webp가 가장 작고, png는 webp를 못 만드는 기기의 대비책입니다
   *   (칸수를 줄여 크기를 맞춰 보냅니다). */
  return /^data:image\/(webp|png);base64,[A-Za-z0-9+/=]+$/.test(v);
}
