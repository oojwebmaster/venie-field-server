 ...
 *
 * ## ★★ 세 줄 요약
 * 1. **저장되는 것은 언제나 webp 한 장**입니다. png가 올라오면 여기서
 *    바꿔서 저장합니다 — iOS는 webp를 만들지 못하기 때문입니다(16.4 아래는
 *    `toDataURL('image/webp')`에 말없이 png를 돌려주고, 아이폰의 크롬·
 *    파이어폭스도 속은 WebKit이라 똑같습니다).
...
export class Gallery {
  ...
  constructor(opt = {}) {
    ...
    /**
     * ★★ png → webp로 바꿔 주는 사람(`sharp`). 없으면 webp가 아닌 것은
     *   받지 않습니다 — `.webp`라는 이름의 png가 쌓이는 것보다 낫습니다.
     * @type {null|((buf:Buffer)=>Promise<Buffer>)}
     */
    this.convert = opt.convert || null;
    ...
    /**
     * ★★ 저장소에 **못 쓴** 그림을 여기 붙듭니다.
     *
     *   저장에 실패해도 작품은 살려 둡니다(사람이 몇 분 그린 것이니까요).
     *   그런데 그림을 저장소에서만 꺼내면, 살려 둔 그 작품은 **하얗게**
     *   섭니다 — 이젤은 서 있는데 그림이 없습니다. 그 판이 끝날 때까지는
     *   메모리에서 내어 줍니다.
     */
    this._mem = new Map();
   ...
   */
  async putImage(id, ticket, raw, now = Date.now()) {
    let buf = raw;
    ...
    /* ★★ **여기가 webp를 보장하는 곳입니다.**
     *   게임 쪽 확인만 믿으면 png가 `.webp` 이름으로 쌓입니다. 머리글을
     *   직접 보고, webp가 아니면 **바꿔서** 저장합니다. */
    if (!isWebp(buf)) {
      if (!this.convert) return { ok: false, why: 'noconv' };
      let made = null;
      try {
        made = await this.convert(buf);
      } catch (e) {
        this.log(`webp로 바꾸지 못했습니다: ${e.message}`);
        return { ok: false, why: 'convert' };
      }
      if (!isWebp(made)) return { ok: false, why: 'convert' };
      this.log(`png ${(buf.length / 1024).toFixed(0)}KB → webp ${(made.length / 1024).toFixed(0)}KB`);
      buf = made;
    }
    ...
    try {
      ...
      if (this._mem.size > 16) this._mem.delete(this._mem.keys().next().value);
      this._mem.set(id, buf);
    }
    /* 방금 올린 그림은 **곧바로 달라고 합니다**(세우자마자 화면에 그립니다).
     * 한 번은 저장소를 다녀오지 않도록 손에 들고 있습니다. */
    this._cache = { id, buf };
   ...
   *
   * ★★ `m`(mine) — **이 사람의 작품인가를 서버가 판정해서 보냅니다.**
   *   게임 쪽에서 이름을 견주게 두면, 두 곳에서 **따로 다듬은 문자열**을
   *   맞춰 보는 셈이 됩니다(서버는 `sanitizeName`+`ownerKey`, 게임은 제
   *   나름의 정리). 한 글자만 어긋나도 **내 작품에 회수 버튼이 안 뜨고,
   *   오류는 어디에도 남지 않습니다.** 주인을 아는 곳은 여기 한 곳뿐이니
   *   여기서 답을 줍니다.
   * @param {string} [viewerKey] 보는 사람의 열쇠꼴 이름
   */
  static wire(m, viewerKey) {
    return {
      i: m.id, t: m.title, o: m.owner, x: m.x, z: m.z, r: m.r || 0, s: m.state,
      m: viewerKey && m.key === viewerKey ? 1 : 0,
    };
  }
  ...
  visibleTo(owner) {
    ...
    for (const m of this.items.values()) {
      ...
      if (m.state === ART.APPROVED) { out.push(Gallery.wire(m, key)); continue; }
      if (key && m.key === key) out.push(Gallery.wire(m, key));
    }
    return out;
  }
  ...
  heldBy(owner) {
    ...
    return best ? Gallery.wire(best, key) : null;
  }
