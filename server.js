...
/** 한 그림의 최대 크기(바이트). 512칸 webp는 보통 40~150KB입니다 */
const ART_BYTES = Number(process.env.ART_MAX_BYTES) || 300 * 1024;
...
const server = http.createServer(async (req, res) => {
  ...
  /* ── 그림 올리기 (자리표가 있어야 합니다) ──────────────────────────── */
  if (req.method === 'POST' && /^\/art\/[\w-]{6,32}$/.test(path)) {
    cors(req, res);
    ...
    let raw;
    try {
      /* base64라 원본의 4/3입니다. `text/plain`으로 보내면 브라우저가
       * 사전 요청(preflight)을 안 하므로 왕복이 한 번 줄어듭니다. */
      raw = await readBody(req, Math.ceil(ART_BYTES * 4 / 3) + 1024);
    } catch (e) {
      json(res, 413, { ok: false, why: 'size' });
      return;
    }
    const buf = Buffer.from(raw.toString('utf8').replace(/^data:[^,]*,/, ''), 'base64');
    if (buf.length > ART_BYTES) { json(res, 413, { ok: false, why: 'size' }); return; }
    const r = await gallery.putImage(id, url.searchParams.get('tk') || '', buf);
    if (!r.ok) { json(res, 400, r); return; }
    console.log(`[작품] 올라옴 ${id} · ${(buf.length / 1024).toFixed(0)}KB`);
    json(res, 200, r);
    return;
  }
  ...
  if (path === '/art/admin/api' && req.method === 'POST') {
    ...
    if (body.a === 'approve') {
      ...
      if (m.state === ART.APPROVED) broadcast({ t: 'art', a: 'set', v: Gallery.wire(m) });
...
async function handleArt(ws, m) {
  ...
  if (m.a === 'new') {
    /* ★ 이름 필터를 **여기서 다시** 봅니다. 게임 쪽에서도 거르지만 그 검사는
     *   고쳐서 보낼 수 있고, 작품명은 남의 화면에 오래 남습니다. */
    if (isBadName(String(m.ti || ''))) { send(ws, { t: 'art', a: 'err', why: 'title' }); return; }
    const r = gallery.reserve(who, m.ti, m.pin);
    if (!r.ok) { send(ws, { t: 'art', a: 'err', why: r.why }); return; }
    send(ws, { t: 'art', a: 'slot', i: r.id, tk: r.ticket });
    return;
  }
  ...
  if (m.a === 'place') {
    ...
    send(ws, { t: 'art', a: 'set', v: Gallery.wire(r.meta) });
    console.log(`[작품] 세움 '${r.meta.title}' (${r.meta.owner})`
      + ` — ${r.meta.state === ART.APPROVED ? '승인됨' : '승인 대기'}`);
    /* 승인된 작품을 옮겼다면 남들의 화면에서도 옮겨져야 합니다 */
    if (r.meta.state === ART.APPROVED) broadcast({ t: 'art', a: 'set', v: Gallery.wire(r.meta) }, ws.id);
    return;
  }
