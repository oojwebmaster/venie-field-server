 ...
 *
 * 예전에는 이랬습니다:
 *   ① 깨어나며 `loadBoard()`가 Upstash를 읽는데 실패합니다
 *      (콜드 스타트·순간 요청 제한·500 — 무료 플랜에서 드물지 않습니다)
 *   ② `res.ok`를 안 봐서 `catch`에도 안 걸리고, `rows`가 배열이 아니라
 *      **조용히 아무 일도 안 합니다.** 콘솔에 한 줄도 안 남습니다
 *   ③ `room.board`는 빈 채로 남고, 누가 점수를 얻으면 `saveBoardSoon()`이
 *      **한 줄짜리 배열로 지난 기록 전체를 덮어씁니다**
 *
 * '없는 것'과 '못 읽은 것'은 전혀 다른 말인데, 코드가 둘을 같게 다뤘습니다.
 */
let boardLoaded = false;
let warnedNoSave = false;

async function loadBoard(tries = 3) {
  if (!storeOn) { boardLoaded = true; return true; }   // 저장소가 없으면 덮어쓸 것도 없습니다
  for (let i = 1; i <= tries; i++) {
    try {
      const res = await fetch(`${STORE_URL}/get/${encodeURIComponent(STORE_KEY)}`, {
        headers: { Authorization: `Bearer ${STORE_TOKEN}` },
      });
      /* ★★ 이 한 줄이 없었습니다. 500이 와도 아래로 흘러가 '기록 없음'이
       *   되었습니다. */
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const j = await res.json();
      /* ★ 키가 아직 없으면(`null`) 그건 **정상적으로 비어 있는 것**입니다 —
       *   처음 켜는 서버가 여기 걸려 영영 저장을 못 하면 안 됩니다. */
      const rows = j?.result ? JSON.parse(j.result) : [];
      if (!Array.isArray(rows)) throw new Error('모양이 배열이 아닙니다');
      room.board = rows.filter((e) => e && typeof e.name === 'string'
        && Number.isFinite(e.score));
      boardLoaded = true;
      console.log(`순위표를 불러왔습니다 — ${room.board.length}줄`);
      return true;
    } catch (e) {
      console.log(`순위표를 불러오지 못했습니다 (${i}/${tries}):`, e.message);
      if (i < tries) await new Promise((r) => setTimeout(r, i * 1500));
    }
  }
  console.log('★★ 순위표를 끝내 못 읽었습니다 — 덮어쓰지 않도록 **저장을 잠급니다.**'
    + ' 서버를 다시 켜면 다시 시도합니다.');
  return false;
}
...
function saveBoardSoon() {
  ...
  /* ★★ 못 읽었으면 쓰지 않습니다. 빈 순위표로 지난 기록을 덮는 것은
   *   되돌릴 수 없습니다 — 저장을 거르는 쪽이 언제나 낫습니다. */
  if (!boardLoaded) {
    if (!warnedNoSave) {
      warnedNoSave = true;
      console.log('★ 순위표를 아직 못 읽어서 저장을 건너뜁니다 (기록을 지키기 위해)');
    }
    return;
  }
 ...
 * ★ `ART_PIN_SALT`가 없으면 등록을 받지 않습니다. PIN을 평문으로 둘 바에는
 *   기능을 끄는 편이 낫습니다 — 있는 줄 알았던 자물쇠가 없는 것이 제일
 *   나쁩니다. 시작할 때 콘솔에 한 줄 남깁니다. */
...
const server = http.createServer((req, res) => {
  ...
  /* ── ★★ 작품 그림 ── `GET /art/<id>.webp`
   * 소켓으로 보내면 512² webp(40~150KB)를 **접속할 때마다** 다시 받습니다.
   * 여기로 내주면 브라우저가 캐시합니다. 작품은 한 번 올라오면 내용이
   * 안 바뀌므로 `immutable`을 붙여도 안전합니다. */
 ...
 *
 * ★★ 예전에는 2KB에서 무조건 끊었습니다. 그래서 **작품을 세우는 순간
 *   접속이 통째로 죽었고**, 순위표까지 사라져 보였습니다(순위표는 서버가
 *   주는 것이니까요).
 * ★★ 그렇다고 상한을 통째로 올리면 아무 메시지나 600KB로 보내도 되는
 *   문이 열립니다. 무료 플랜은 그 문 하나로 대역폭이 다 탑니다.
 *   그래서 **두 단계**로 봅니다 — 큰 것은 '작품 등록'일 때만.
 * ★ `maxPayload`도 함께 올립니다. 안 올리면 `ws`가 프레임 단계에서 먼저
 *   끊어, 아래 검사가 돌기도 전에 연결이 죽습니다. */
const MSG_MAX = 2048;
const ART_MSG_MAX = Number(process.env.ART_MSG_MAX) || 600 * 1024;
/** 작품을 올리고 이만큼은 다시 못 올립니다(ms). 그리는 데 몇 분 걸립니다 */
  ...
  ws.on('message', (raw) => {
    ...
    if (m.t === 'hello') {
      ...
      ws.send(JSON.stringify({
        ...
        /* ★★ 중복을 피해 붙인 번호를 **뗀** 이름('돈 2' → '돈'). 작품의
         *   주인은 이 이름으로 가립니다 — 새로고침할 때 앞 접속이 잠깐
         *   남으면 번호가 붙는데, 그걸로 가리면 **자기 작품이 남의 것이
         *   되어 사라집니다.** 순위표가 `baseName`으로 기록하는 것과 같습니다. */
        ...
        np: room.npcBusyList(),
        /* 승인된 작품 + **내 작품**. 그림은 `/art/<id>.webp`로 따로 받습니다 */
 ...
 *   a: 'take'  회수 — 주인이고 PIN이 맞아야
 *   a: 'list' | 'ok' | 'no' | 'del'   관리자 (열쇠 필요)
 */
...
async function handleArt(ws, m) {
  ...
  if (m.a === 'new') {
    ...
    const r = await gallery.add({
      ...
      title: m.t2, pin: m.p, img: m.img,
      x: m.x, y: m.y, z: m.z, rotY: m.r,
    });
    if (!r.ok) { ws.send(JSON.stringify({ t: 'art', a: 'denied', why: r.why })); return; }
  ...
  if (m.a === 'take') {
     ...
     *   주지 않습니다 — 남의 작품 PIN을 만 번 두드리는 길이 막힙니다. */
  ...
  if (m.a === 'ok' || m.a === 'no' || m.a === 'del') {
    ...
    if (m.a === 'ok') {
      ...
      /* ★ 거절도 **삭제**입니다. 남겨 두면 언젠가 실수로 승인될 수 있고,
       *   그림은 그대로 저장소를 차지합니다. */
      await gallery.remove(m.id);
      broadcast({ t: 'art', a: 'gone', id: m.id });
      console.log(`작품 ${m.a === 'no' ? '거절' : '삭제'} — '${art.title}' by ${art.owner}`);
    }
    ws.send(JSON.stringify({ t: 'art', a: 'list', ...gallery.adminList() }));
  }
}
