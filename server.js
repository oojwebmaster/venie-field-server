...
/* ★ 12 → 20. 한 칸이 83ms → 50ms가 되어 점프처럼 짧은 동작이
 * '한 박자 늦게' 보이던 것이 눈에 띄게 줄어듭니다.
 * 클라이언트의 `config.net.sendRate`와 같은 값이어야 합니다. */
...
/* ── 물감 ────────────────────────────────────────────────────────────────
 * ★ **서버가 진실을 가집니다.** 어느 자리에 몇 개가 떠 있는지, 누가 먹었는지를
 *   여기서 한 번만 정합니다. 클라이언트가 각자 "내가 먹었다"를 믿으면
 *   두 사람이 같은 물감을 먹고 둘 다 색이 바뀝니다.
 *
 * ★ 다만 **좌표는 서버가 만들지 않습니다.** 지형은 시드 기반 결정적 생성이라
 *   서버가 들고 있을 이유가 없고, 그렇다고 여기서 지형을 굽는 것은 무료 플랜에
 *   과합니다. 대신 오프라인에서 구워 둔 후보표(spawns.js)를 클라이언트와
 *   **같은 파일로** 나눠 갖고, 서버는 그 **번호만** 고릅니다.
 *   (server/spawns.js 와 src/spawns.js 는 완전히 같아야 합니다)
 */
const ITEM_COUNT = Number(process.env.ITEM_COUNT) || 14;
const ITEM_RESPAWN = (Number(process.env.ITEM_RESPAWN) || 60) * 1000;
const ITEM_SCORE = Number(process.env.ITEM_SCORE) || 1;

/* ── 조개 ────────────────────────────────────────────────────────────────
 * 물감과 **표가 다릅니다**(shells.js). 물감표는 해수면 +1.5m 위만 담고 있어
 * 모래사장이 통째로 빠져 있고, 조개는 정확히 그 빠진 자리에 놓입니다.
 * 그래서 개수·리스폰 시간·자리 번호를 따로 가집니다.
 *
 * ★ `Number(x) || 기본값`은 **0을 기본값으로 되돌립니다.** 일반 조개 점수는
 *   0이 정상적인 값이므로(진주만 점수를 준다는 선택) 여기서는 쓰면 안 됩니다.
 *   `??`로 "설정하지 않았을 때만" 기본값이 되게 합니다. */
const numEnv = (name, fallback) => {
  const v = Number(process.env[name]);
  return Number.isFinite(v) ? v : fallback;
};
const SHELL_COUNT = numEnv('SHELL_COUNT', 6);
const SHELL_RESPAWN = numEnv('SHELL_RESPAWN', 120) * 1000;
const SHELL_SCORE = numEnv('SHELL_SCORE', 0);
const PEARL_SCORE = numEnv('PEARL_SCORE', 5);
const PEARL_CHANCE = numEnv('PEARL_CHANCE', 0.1);
const PEARL_MAX = numEnv('PEARL_MAX', 1);
...
const room = new Room({
  ...
  itemRespawn: ITEM_RESPAWN,
  itemColors: PAINTS.length,
  itemScore: ITEM_SCORE,
  spawnCount: SPAWN_COUNT,
  claimRange: Number(process.env.ITEM_CLAIM_RANGE) || 12,
  spawnAt,
  shellCount: SHELL_COUNT,
  shellRespawn: SHELL_RESPAWN,
  shellScore: SHELL_SCORE,
  pearlScore: PEARL_SCORE,
  pearlChance: PEARL_CHANCE,
  pearlMax: PEARL_MAX,
...
server.listen(PORT, () => {
  ...
  console.log(`물감 ${ITEM_COUNT}개 · ${ITEM_RESPAWN / 1000}초 후 리스폰`
    + ` · 개당 ${ITEM_SCORE}점 · 후보 ${SPAWN_COUNT}자리`);
  ...
  console.log(`조개 ${SHELL_COUNT}개 · ${SHELL_RESPAWN / 1000}초 후 리스폰`
    + ` · 진주 확률 ${(PEARL_CHANCE * 100).toFixed(0)}% (최대 ${PEARL_MAX}개)`
    + ` · 조개 ${SHELL_SCORE}점 / 진주 ${PEARL_SCORE}점 · 후보 ${SHELL_SPAWN_COUNT}자리`);
