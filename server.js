...
/* ── ★★ 4단계 갤러리 ──────────────────────────────────────────────────────
 * 순위표와 **같은 Upstash**를 씁니다(열쇠만 다릅니다). 저장소를 하나 더 두면
 * 환경변수도 하나 더 늘고, **둘 중 하나만 설정된 상태**가 반드시 생깁니다.
 *
 * ★ `Gallery`는 `Room`도 `ws`도 모릅니다 — 그림을 받아 두고 물어보면 내어
 *   주는 것이 전부입니다. 방(사람·아이템)과 갤러리는 **수명이 다릅니다**:
 *   사람은 나가면 사라지지만 작품은 남습니다.
 */
const kv = storeOn ? {
  async get(key) {
  ...
  async set(key, val) {
    /* ★ 그림은 십만 자가 넘습니다. 주소에 넣으면 URL 길이 제한에 걸리므로
     *   **몸통으로** 보냅니다 — 순위표가 쓰는 방식과 같습니다. */
    await fetch(`${STORE_URL}/set/${encodeURIComponent(key)}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${STORE_TOKEN}` },
      body: val,
    });
  ...
  async del(key) {
    await fetch(`${STORE_URL}/del/${encodeURIComponent(key)}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${STORE_TOKEN}` },
    });
  },
...
const gallery = new Gallery({ store: kv });
/* ★ 바뀔 때마다 쓰면 요청이 폭발합니다 — 순위표와 같은 이유로 미룹니다.
 *   그림은 만들 때 한 번만 쓰므로 미루지 않습니다(잃으면 되살릴 수 없습니다). */
let artTimer = null;
gallery.onChange = () => {
  if (!kv || artTimer) return;
  artTimer = setTimeout(() => { artTimer = null; gallery.saveMeta(); }, 4000);
};
...
/* ── ★★ 작품 (4단계) ─────────────────────────────────────────────────────
 * `t: 'art'` 하나로 세우기·회수·관리를 모두 받습니다. 메시지 종류를 여럿
 * 만들면 클라이언트와 서버의 이름이 어긋나기 쉽고, 어긋나도 **아무 말이
 * 없습니다**(그냥 아무 일도 안 일어납니다).
 */
function handleArt(ws, m) {
  const me = room.players.get(ws.id);
  if (!me) return;
  const reply = (o) => ws.send(JSON.stringify({ t: 'art', ...o }));

  if (m.a === 'place') {
    const r = gallery.place(me.name, m);
    if (!r.ok) { reply({ a: 'denied', op: 'place', why: r.why }); return; }
    /* ★ 세운 본인에게는 곧바로 보입니다(승인 전이라도). 남에게는 승인
     *   뒤에야 갑니다 — 그때 `admin`이 전파합니다. */
    reply({ a: 'add', art: gallery.wire(r.art, false), mine: true });
    console.log(`+ 작품 "${r.art.title}" — ${me.name} (승인 대기 ${gallery.pendingCount}점)`);
    return;
  }

  if (m.a === 'recall') {
    const r = gallery.recall(ws.id, me.name, m.id, String(m.pin || ''));
    if (!r.ok) { reply({ a: 'denied', op: 'recall', id: m.id, why: r.why }); return; }
    /* 걷어 간 사람에게는 그림을 돌려줍니다 — 다시 머리에 얹어야 하니까요 */
    reply({ a: 'recalled', id: r.art.id, title: r.art.title, img: r.img });
    broadcast({ t: 'art', a: 'del', id: r.art.id });
    return;
  }

  /* ── 관리자 ─────────────────────────────────────────────────────────
   * ★★ 개발자모드 화면을 막는 것만으로는 부족합니다. 소켓으로 이 메시지를
   *   직접 보내면 그만이라, **서버가 열쇠를 다시 봅니다.** */
  if (m.a === 'admin') {
    if (!ADMIN_KEY || m.k !== ADMIN_KEY) {
      reply({ a: 'denied', op: 'admin', why: ADMIN_KEY ? '관리자 키가 다릅니다' : 'ADMIN_KEY 미설정' });
      return;
    }
    if (m.op === 'list') { reply({ a: 'list', arts: gallery.adminList() }); return; }
    if (m.op === 'approve') {
      const a = gallery.approve(m.id);
      if (!a) { reply({ a: 'denied', op: 'admin', why: '없거나 이미 승인된 작품입니다' }); return; }
      /* ★ 승인되면 **그때 모두에게** 보냅니다 — 그림까지 함께 */
      broadcast({ t: 'art', a: 'add', art: gallery.wire(a, true) });
      reply({ a: 'list', arts: gallery.adminList() });
      console.log(`✔ 승인 "${a.title}" — ${a.owner}`);
      return;
    }
    if (m.op === 'remove') {
      const a = gallery.remove(m.id);
      if (!a) { reply({ a: 'denied', op: 'admin', why: '없는 작품입니다' }); return; }
      broadcast({ t: 'art', a: 'del', id: a.id });
      reply({ a: 'list', arts: gallery.adminList() });
      console.log(`✖ 내림 "${a.title}" — ${a.owner}`);
      return;
    }
    reply({ a: 'denied', op: 'admin', why: '모르는 명령입니다' });
    return;
  }

  if (m.a === 'img') {
    /* 그림만 따로 달라 — 승인된 작품이 늦게 필요해질 때 */
    const art = gallery.arts.get(m.id);
    if (!art || (!art.approved && art.owner !== me.name)) return;
    reply({ a: 'img', id: m.id, img: gallery.images.get(m.id) || null });
  }
}
...
const server = http.createServer((req, res) => {
  if (req.url === '/health' || req.url === '/') {
    ...
    res.end(JSON.stringify({
      ...
      /* ★ 브라우저로 이 주소를 열면 **어떤 판이 돌고 있는지** 한눈에 보입니다.
       *   `gallery` 칸이 없으면 4단계 이전 서버입니다 — "작품을 세웠는데
       *   승인함이 비어 있다"의 첫 번째 확인 지점입니다. */
      gallery: {
        total: gallery.arts.size,
  ...
  ws.on('message', (raw) => {
    ...
    if (m.t === 'hello') {
      ...
      ws.send(JSON.stringify({
        ...
        np: room.npcBusyList(),
        /* ★ 지금 서 있는 작품들. 승인된 것 + **내가 세운 것**입니다 —
         *   승인 전인 내 작품이 안 보이면 어디에 뒀는지 몰라 걷을 수도
         *   옮길 수도 없습니다. */
        art: gallery.listFor(r.player.name),
      }));
...
server.listen(PORT, () => {
  ...
  if (storeOn) {
    console.log(`· 순위표와 갤러리를 바깥 저장소에 보관합니다 (${STORE_URL})`);
    loadBoard();
    gallery.load();
  } else {
    console.log('· ★ 순위표와 갤러리가 메모리에만 있습니다 — 무료 플랜은 15분 놀면'
      + ' 잠들고, 깨어날 때 기록이 사라집니다. SCORE_STORE_URL/TOKEN을 넣으면 보관됩니다');
  }
  if (!ADMIN_KEY) {
    console.log('· ★★ ADMIN_KEY가 없으면 작품을 **승인할 수 없습니다** —'
      + ' 세운 작품이 본인에게만 보이는 채로 쌓입니다');
  }
