 ...
 * ★★ **읽어 본 적이 없으면 쓰지 않습니다.**
 *
 * 이 깃발이 없었을 때 순위 기록이 통째로 날아갔습니다. 순서가 이랬습니다:
 *
 *   ① 서버가 깨어나며 `loadBoard()`가 Upstash를 읽는데 **실패합니다**
 *      (콜드 스타트·순간적인 요청 제한·500 — 무료 플랜에서 드물지 않습니다)
 *   ② `room.board`는 빈 배열 그대로 남습니다
 *   ③ 누군가 물감을 하나 먹습니다 → `saveBoardSoon()`
 *   ④ 8초 뒤, **한 줄짜리 배열이 지난 기록 전체를 덮어씁니다**
 *
 * 되돌릴 수 없는 사고인데 콘솔에는 한 줄도 안 남았습니다. `loadBoard()`가
 * `res.ok`를 안 봐서, 서버가 500을 주면 `catch`에도 안 걸리고 그냥
 * '빈 순위표'로 조용히 넘어갔기 때문입니다.
 */
let boardLoaded = false;
let warnedNoLoad = false;
...
async function loadBoard(tries = 3) {
  ...
  for (let i = 1; i <= tries; i++) {
    try {
      ...
      /* ★★ 여기가 비어 있었습니다. 500이나 요청 제한이 와도 아래로 그냥
       *   흘러가 '기록 없음'이 되었습니다 — **없는 것과 못 읽은 것은
       *   전혀 다른 말입니다.** */
  ...
  console.log('★★ 순위표를 끝내 못 읽었습니다 —'
    + ' 덮어쓰지 않도록 **저장을 잠급니다.** 서버를 다시 켜면 재시도합니다.');
  return false;
}
...
function saveBoardSoon() {
  ...
  /* ★★ **읽기에 실패했으면 쓰지 않습니다.** 지금 들고 있는 것은 '지난
   *   기록'이 아니라 '이번에 켜진 뒤 생긴 것'뿐이라, 그대로 쓰면 지난
   *   기록을 지웁니다. 점수 몇 줄을 잃는 것과 전체를 잃는 것은 다릅니다. */
  if (!boardLoaded) {
    if (!warnedNoLoad) {
      warnedNoLoad = true;
      console.log('★ 순위표를 못 읽은 상태라 저장을 건너뜁니다 (덮어쓰기 방지)');
    }
    return;
  }
...
server.listen(PORT, () => {
  ...
  if (storeOn) {
    ...
    /* ★ 순서대로 부릅니다. 둘을 한꺼번에 던지면 무료 플랜의 순간 요청
     *   제한에 함께 걸릴 수 있고, 그때 지는 쪽이 순위표입니다. */
    loadBoard().then(() => gallery.load());
  } else {
