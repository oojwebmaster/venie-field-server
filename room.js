...
export class Room {
  ...
  join(name, now = Date.now()) {
    ...
    const player = {
      ...
      sw: 0,      // 수영 블렌드 0~1
      air: 0,     // 공중(점프) 0/1
      /* 이번 접속의 누적 점수. 저장하지 않으므로 나갔다 오면 0부터입니다 */
      score: 0,
  ...
  addScore(id, points) {
    ...
    p.score += points;
    this._record(p.name, p.score);
    return p.score;
  }

  /**
   * 이름 하나당 한 줄. 같은 이름으로 다시 들어오면 **더 높은 점수만** 남깁니다
   * (같은 이름을 여러 명이 쓸 수 있지만, 순위표는 이름으로 보는 것이라
   *  줄을 나누면 같은 이름이 여러 번 뜨는 쪽이 더 이상해집니다).
   */
  _record(name, score) {
    if (this.banned.has(name)) return;
  ...
  /**
   * 관리자가 기록 한 줄을 지웁니다 (필터를 피한 비속어 닉네임 대응).
   * ★ 지우기만 하면 그 사람이 계속 놀고 있는 한 다음 점수에서 곧바로 다시
   *   올라옵니다. 그래서 이름을 막아 둡니다.
   */
  removeScore(name) {
  ...
  _spawn(color) {
    ...
    const it = {
      ...
      // 아이템이 자기 점수를 들고 있습니다 — 나중에 종류가 늘어도 그대로 돕니다
      score: O.itemScore,
      born: Date.now(),
    };
    this.items.push(it);
  ...
  /** 전선에 싣는 형태 — i(아이디) · s(스폰 번호) · c(색 번호) · v(점수) */
  static itemWire(it) {
    return { i: it.id, s: it.spawn, c: it.color, v: it.score };
  }
